...
ffi.cdef[[
static const int LBER_ERROR   =0xffffffff;
static const int LBER_DEFAULT =0xffffffff;
]]
...
--[[
    The BerElement, and BERVAL structure was taken from the wldap32.lua file and placed
    here.  This is to remove a circular dependency between the two files.
    It makes more sense to be defined here because the routines that manipulate
    the structure are also in this file, which makes it fairly independent.
--]]
ffi.cdef[[
//
...
NULLBER = ffi.cast("BerElement *", nil);
...
ffi.cdef[[
ULONG ber_scanf( BerElement *pBerElement, PCHAR fmt, ... );
]]
